...
edges2 = hist_sm2.xEdges()
vals_lo2 = hist_sm2.areas()
print("vals lo despoina", vals_lo2, "XS (in fb)", np.sum(vals_lo2))
#
#
...
# 
print("xEdges agree?: ",  edges - edges2 )
print("vals_lo agree?: ",  vals_lo/vals_lo2  )
#
#
# 
file_eft = yoda.read('cW_WZ_dib_linear.yoda')
hist_eft = file_eft ['/WZ_ATLAS/d12-x01-y01']
vals_eft = (hist_eft.areas() - vals_lo)
print("vals EFT ", vals_eft, "XS lin EFT (in fb)", np.sum(vals_eft))
# 
file_eft2 = yoda.read('despoina_yodas/cW.yoda')
hist_eft2 = file_eft2 ['/TESTDET/d12-x01-y01']
vals_eft2 = (hist_eft2.areas() - vals_lo2)
print("vals EFT despoina ", vals_eft2, "XS lin EFT (in fb)", np.sum(vals_eft2))

print("vals_eft agree?: ",  vals_eft/vals_eft2 )
...
def plot_lin():
    ...
    xData_of = edges[-2] + (edges[-1] - edges[-2])/2
    ...
    if not os.path.exists('sensitivityplots_desp/PNG'):
        os.makedirs('sensitivityplots_desp/PNG')
    if not os.path.exists('sensitivityplots_desp/PDF'):
        os.makedirs('sensitivityplots_desp/PDF/')
    plt.savefig('sensitivityplots_desp/PDF/WZ_atlas_MtWZ.pdf')
    plt.savefig('sensitivityplots_desp/PNG/WZ_atlas_MtWZ.png')
# # end def
...
def plotSens(interf, op, factor, label):
    ...
    if not os.path.exists('sensitivityplots2_desp/PNG'):
        os.makedirs('sensitivityplots2_desp/PNG')
    if not os.path.exists('sensitivityplots2_desp/PDF'):
        os.makedirs('sensitivityplots2_desp/PDF/')
    plt.savefig('sensitivityplots2_desp/PDF/signal_only_'+op+label+'.pdf')
    plt.savefig('sensitivityplots2_desp/PNG/signal_only_'+op+label+'.png')
# end def
